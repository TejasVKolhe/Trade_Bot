...
def get_curr_price_data(ticker, curr_interval):    
    curr_data=ticker.history(period='1d', interval=f"{curr_interval}m")
    curr_data['EMA12']=curr_data['Close'].ewm(span=12, adjust=False).mean()
    curr_data['EMA26']=curr_data['Close'].ewm(span=26, adjust=False).mean()
    curr_data['MACD']=curr_data['EMA12']-curr_data['EMA26']
    curr_data['SGN']=curr_data['MACD'].ewm(span=9, adjust=False).mean()
    return curr_data


def plot_curr_data(curr_data):
